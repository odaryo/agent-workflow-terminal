...
- **合成キーストロークは埋め込み端末 (libghostty の NSView) へ届く。** 以前「届かないので実キーボードを
  代表できない」と記録されていたが誤りで、`verify-app-ui.sh type` は端末へも入る。「実キーボードでしか
  測れない」と早々に諦めないこと。ただし System Events の制約で**非 ASCII は送れない**。逆に AX からは、
  libghostty の NSView が Accessibility 要素として現れないため「端末がフォーカスを持っている」と
  「どこも持っていない」を区別できない — フォーカスの所在は `focused` の戻り値ではなく**打鍵の到達先**で
  判定する。
- **日本語 IME が有効だと、合成打鍵は未確定の変換バッファに溜まり `capture-pane` に現れない。**
  pty へ渡っていないので tmux 側からは「1文字も届いていない」と完全に同じに見える。#316 の実機 E2E で
  2度これに騙され、「端末が打鍵を受け取れない」という実在しない欠陥を報告しかけた — 後にユーザーの
  実打鍵が確定させ、溜まっていた合成打鍵がまとめて pane に出た (`RTQ4ROOTあｓｄPQ8TESTLIVEKEY123`)。
  上の「旧計測の null 結果の原因は特定できていない」もおそらくこれである。**打鍵を計測する前に英数入力へ
  切り替えるか、pane の null 結果を結論にする前に陽性対照 (同じ手順で届くはずの打鍵) を取ること。**
- **⌘ を伴うショートカットは合成打鍵で効かないことがある。** #316 で `⌘W` は System Events の
  `keystroke "w" using {command down}` では何も起こらず、**実キーボードでは window が閉じてアプリが
  終了した**。同じ動作をメニュー項目のクリック (`click menu item "Close" of ...`) では再現できたので、
  **ショートカットの検証は「メニュー項目が動くこと」と「実キーで効くこと」に分けて測る**。合成打鍵の
  null 結果を「ショートカットが壊れている」と読まないこと。

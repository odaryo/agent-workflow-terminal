...
- 外部プロセスの実行中に呼び出し元の `Task` がキャンセルされた場合は、子プロセスへ終了シグナルを
  送り、終了を待ち受けてからキャンセルエラーを返す。子プロセスを残したままにしない。
- キャンセル後に子プロセスが終了コード 0 で終わっても、成功結果へ置き換えない。キャンセルは
  呼び出し元が処理の継続を不要とした制御情報であり、握り潰すと後続処理が誤って進むため。
...
- stdout と stderr は並行に最後まで読み切る。一方を先に待つと、他方のパイプバッファが満杯になり、
  子プロセスが書き込み待ちのまま停止してデッドロックするため。
- 汎用実行層は非ゼロの終了コードを結果値として返し、stderr も成功・失敗にかかわらず保持する。
  非ゼロ終了をエラーへ変換するかは、正常系でも stderr を使う CLI があるため、CLI ごとの Adapter が
  決める。
- 子プロセスへ渡す環境変数は Adapter が明示的に組み立て、親プロセスの環境を暗黙に継承しない。
- タイムアウト時は SIGTERM、短い猶予、SIGKILL の順で終了させ、必ず終了を待ち受ける。終了確認を
  省くと子プロセスがゾンビとして残るため。Task キャンセル時は §1.4 に従う。
- 実行バイナリの解決は §5.2 のとおり Adapter に一元化する。

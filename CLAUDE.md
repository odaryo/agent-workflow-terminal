...
| Carry the previous session's final response across `/clear` (hooks invoke it automatically; humans must not run it) | `scripts/wf-handoff.sh` |
...
- handoff は `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/handoff/<key>/` に置かれる。`<key>` は worktree ごとの `git rev-parse --show-toplevel` の絶対パスの `/` を `-` に置換し、元の絶対パスの SHA-256 先頭8文字を加えたものなので、並行する worktree 間では共有しない。Stop / SessionEnd hook が直前セッションの最終応答を書き、次の startup / clear の SessionStart hook が読む。hook が自動で行うため Director の操作は不要で、人が実行してはならない。保存先全体は `AWT_HANDOFF_DIR` で差し替えられる。

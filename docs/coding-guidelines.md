...
- 出力をパースするクライアントへ渡す環境変数は Adapter が明示的に組み立て、親環境を暗黙に
  継承しない。tmux server を起動したクライアントの環境は全 pane に継承されるため、server 起動へ
  同じ限定環境を使う方針までは一般化しない。
- タイムアウト時は SIGTERM、短い猶予、SIGKILL の順で直接の子を終了・回収し、出力 pipe は EOF を
  待たずに閉じる。孫が書き込み端を保持すると EOF は子の終了後も来ず、タイムアウトの上界を
  壊すことを実測している。Task キャンセル時は §1.4 に従う。
